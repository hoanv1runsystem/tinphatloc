...
public class UserServiceImpl implements UserService {
    ...
    public UserResponse addVaildate(UserDto dto) throws Exception {
        ...
        UserResponse result = vailidate(dto, true);
    ...
    @Override
    public UserResponse update(UserDto dto) throws Exception {
        Optional<User> option = userRepository.findById(dto.getId());
        //UserResponse response = new UserResponse();

        UserResponse response = vailidate(dto, false);
        if (response.getStatus() != Constant.STATUS_SUCCSESS) {
            return response;
        }
        ...
        if (option.isPresent()) {
            ...
            dataDb.setPassword(passwordEncoder.encode(dto.getPassword()));
            User result = userRepository.save(dataDb);
            BeanUtils.copyProperties(dto, result);
            response.setStatus(Constant.STATUS_SUCCSESS);
            response.setMessage(Message.UPDATE_SUCCESS);
    ...
    private UserResponse vailidate(UserDto dto, boolean isAdd) {
        UserResponse result = new UserResponse();

        if (isAdd && !StringUtils.hasText(dto.getUsername())) {
        ...
        if (isAdd && existsByEmail) {
            result.setStatus(Constant.DUPLICATE);
            result.setMessage(String.format(Message.NAME_DUPLICATE, dto.getEmail()));
            result.setUser(dto);
            return result;
        }

        if (!isAdd) {
            List<User> userList = userRepository.findByEmail(dto.getEmail());
            ...
            for (User u : userList) {
                if (!Objects.equals(u.getUsername(), dto.getUsername())
                    && Objects.equals(u.getEmail(), dto.getEmail())) {
                    result.setStatus(Constant.DUPLICATE);
                    result.setMessage(String.format(Message.NAME_DUPLICATE, dto.getEmail()));
                    result.setUser(dto);
                    return result;
