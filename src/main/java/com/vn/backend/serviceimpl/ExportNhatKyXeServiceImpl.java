...
@Service
public class ExportNhatKyXeServiceImpl implements ExportNhatKyXeService {
    ...
    @Override
    public List<ExportNhatKyXeDto> findAll(RequestExportDto dto, Integer deleteFlag) {

        return reportCustomRepository.findAll(dto, deleteFlag);
    }

    @Override
    public ExportNhatKyXeResponse viewExport(RequestExportDto dto, Integer deleteFlag) {
        ExportNhatKyXeResponse response = new ExportNhatKyXeResponse();
        List<ExportNhatKyXeDto> dtoList = reportCustomRepository.findAll(dto, deleteFlag);
        response.setExports(dtoList);
        if (!dtoList.isEmpty()) {
            BigDecimal tongSoChuyen = dtoList.stream()
                .map(ExportNhatKyXeDto::getSoChuyen)        // lấy field cần cộng
                .filter(Objects::nonNull)          // tránh null pointer
                .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal tongSoKm = dtoList.stream()
                .map(ExportNhatKyXeDto::getSoKm)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal tongSoTan = dtoList.stream()
                .map(ExportNhatKyXeDto::getSoTan)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal tongKhoiLuong = dtoList.stream()
                .map(ExportNhatKyXeDto::getKhoiLuong)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal tongSoGio = dtoList.stream()
                .map(ExportNhatKyXeDto::getTongGio)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal tongSoCa = dtoList.stream()
                .map(ExportNhatKyXeDto::getSoCa)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);

            BigDecimal tongTien = dtoList.stream()
                .map(ExportNhatKyXeDto::getThanhTien)
                .filter(Objects::nonNull)
                .reduce(BigDecimal.ZERO, BigDecimal::add);
    ...
    @Override
    public byte[] exportFileMutiCondition(RequestExportDto dto, Integer deleteFlag) {
    ...
    @Override
    public byte[] exportFileCondition(RequestExportDto dto, Integer deleteFlag) {
        ClassPathResource templateResource = new ClassPathResource("templates/export/export_temp.xlsx");
        try (InputStream templateStream = templateResource.getInputStream();
            Workbook workbook = new XSSFWorkbook(templateStream);
            ByteArrayOutputStream out = new ByteArrayOutputStream()) {
            ...
            RequestExportDto input = new RequestExportDto();
     ...
     * @return
     */
    private Workbook createDataSheetExport(RequestExportDto dto, Integer deleteFlag, Workbook workbook, Sheet sheetNew, Sheet sheetTemp,
        String title) {

        List<ExportNhatKyXeDto> listDataExport = reportCustomRepository.findAll(dto, deleteFlag);
        ...
        for (ExportNhatKyXeDto data : listDataExport) {
    ...
    private String getTitle(RequestExportDto dto, String condition) {
