...
import com.vn.backend.service.ExportChiPhiService;
...
@Service
public class ExportChiPhiServiceImpl implements ExportChiPhiService {
    ...
    @Override
    public ExportChiPhiResponse viewChiPhi(RequestExportDto dto, Integer deleteFlag) {
        ExportChiPhiResponse response = new ExportChiPhiResponse();
        List<ExportChiPhiDto> dtoList = reportCustomRepository.exportChiPhi(dto, deleteFlag);
        response.setExports(dtoList);

        if (!dtoList.isEmpty()) {

            BigDecimal tongTien = dtoList.stream()
                .map(ExportChiPhiDto::getThanhTien)
